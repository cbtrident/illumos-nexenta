...
static zio_t *
zio_io_to_allocate(metaslab_class_t *mc, int allocator)
{
	zio_t *zio;
	spa_t *spa = mc->mc_spa;
...
zio_dva_throttle(zio_t *zio)
{
	...
	nio = zio_io_to_allocate(zio->io_mc, zio->io_allocator);
...
zio_allocate_dispatch(metaslab_class_t *mc, int allocator)
{
	...
	mutex_enter(&mc->mc_spa->spa_alloc_locks[allocator]);
	zio = zio_io_to_allocate(mc, allocator);
	mutex_exit(&mc->mc_spa->spa_alloc_locks[allocator]);
	if (zio == NULL)
		return;
    ...
    blkptr_t *old_bp, uint64_t size, boolean_t *slog)
{
	...
	if (error != 0 && spa_can_special_be_used(spa) &&
		...
		error = metaslab_alloc(spa, spa_special_class(spa), size, new_bp, 1,
		    txg, old_bp, METASLAB_HINTBP_AVOID, &io_alloc_list, NULL,
		    cityhash4(0, 0, 0, objset) % spa->spa_alloc_count);
...
zio_ready(zio_t *zio)
{
	...
	if (zio->io_error != 0) {
		...
		if (zio->io_flags & ZIO_FLAG_IO_ALLOCATING) {
			...
			zio_allocate_dispatch(zio->io_mc, zio->io_allocator);
		}
	}
...
zio_dva_throttle_done(zio_t *zio)
{
	...
	metaslab_class_throttle_unreserve(pio->io_mc,
	 ...
	 */
	zio_allocate_dispatch(pio->io_mc, pio->io_allocator);
}
