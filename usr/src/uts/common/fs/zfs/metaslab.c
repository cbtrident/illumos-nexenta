...
metaslab_group_alloc(metaslab_group_t *mg, zio_alloc_list_t *zal,
    ...
    int d, int flags, int allocator)
{
	uint64_t offset;
	ASSERT(mg->mg_initialized);

	offset = metaslab_group_alloc_normal(mg, zal, asize, txg,
	    min_distance, dva, d, flags, allocator);
...
top:
	do {
		...
		uint64_t offset = metaslab_group_alloc(mg, zal, asize, txg,
		    distance, dva, d, flags, allocator);
    ...
    zio_alloc_list_t *zal, zio_t *zio, int allocator)
{
	...
	if (mc == spa_special_class(spa) && !BP_IS_METADATA(bp) &&
		...
		error = metaslab_alloc_dva(spa, spa_normal_class(spa),
		    ...
		    flags | METASLAB_USE_WEIGHT_SECONDARY, zal, allocator);
		if (error == 0) {
		...
		for (int d = 0; d < ndvas; d++) {
			error = metaslab_alloc_dva(spa, mc, psize, dva, d,
			    hintdva, txg, flags, zal, allocator);
			if (error != 0) {
				for (d--; d >= 0; d--) {
