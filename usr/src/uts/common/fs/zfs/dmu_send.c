...
dump_write(dmu_sendarg_t *dsp, dmu_object_type_t type,
    uint64_t object, uint64_t offset, int blksz, const blkptr_t *bp,
...
dmu_send_impl_ss(void *tag, dsl_pool_t *dp, dsl_dataset_t *to_ds,
    ...
    boolean_t large_block_ok, int outfd, uint64_t resumeobj,
    uint64_t resumeoff, vnode_t *vp, offset_t *off,
    ...
    int outfd, vnode_t *vp, offset_t *off, boolean_t sendsize)
{
	...
	if (fromsnap != 0) {
		...
		err = dmu_send_impl_ss(FTAG, dp, ds, &zb, is_clone,
		    embedok, large_block_ok, outfd, 0, 0, vp, off, sendsize,
		    NULL);
	} else {
    ...
    vnode_t *vp, offset_t *off)
{
	...
	if (fromsnap != NULL) {
		...
		err = dmu_send_impl(FTAG, dp, ds, &zb, is_clone,
		    embedok, large_block_ok, outfd, resumeobj,
		    resumeoff, vp, off, NULL);
	} else {
