...
xdr_nfsauth_arg(XDR *xdrs, nfsauth_arg_t *argp)
{
	...
	switch (argp->cmd) {
	case NFSAUTH_ACCESS:
		if (!xdr_nfsauth_access_arg(xdrs, &argp->req_u.areq))
			return (FALSE);
		break;

	case NFSAUTH_AUDITINFO:
		if (!xdr_nfsauth_auditinfo_arg(xdrs, &argp->req_u.ureq))
			return (FALSE);
		break;
...
xdr_nfsauth_res(XDR *xdrs, nfsauth_res_t *argp)
{
	...
	switch (argp->cmd) {
	case NFSAUTH_ACCESS:
		if (!xdr_nfsauth_access_res(xdrs, &argp->res_u.ares))
			return (FALSE);
		break;

	case NFSAUTH_AUDITINFO:
		if (!xdr_nfsauth_auditinfo_res(xdrs, &argp->res_u.ures))
			return (FALSE);
		break;
