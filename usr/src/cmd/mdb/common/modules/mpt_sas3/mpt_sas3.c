...
mptsas_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (!(flags & DCMD_ADDRSPEC)) {
		void		*mptsas3_state = NULL;

		if (mdb_readvar(&mptsas3_state, "mptsas3_state") == -1) {
			mdb_warn("can't read mptsas3_state");
			return (DCMD_ERR);
		}
		if (mdb_pwalk_dcmd("genunix`softstate", "mpt_sas3`mptsas3",
		    argc, argv, (uintptr_t)mptsas3_state) == -1) {
...
mptsas_help(void)
{
	mdb_printf("Prints summary information about each mpt_sas3 instance, "
...
static const mdb_dcmd_t dcmds[] = {
	{ "mptsas3", "?[-tpsdD]", "print mpt_sas3 information", mptsas_dcmd,
	    mptsas_help}, { NULL }
};
