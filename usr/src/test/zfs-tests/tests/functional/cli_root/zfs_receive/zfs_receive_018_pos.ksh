...
#	after a series of promotes.
#
#	The test scenario should be repeated due to random sorting of
#	snapshots during the receive phase.
#
# STRATEGY:
...
#	11. Promote the latest clone again.
#	12. Create the fourth recursive snapshot for the source filesystem.
#	13. Generate an incremental recursive stream from the third snapshot
#	    (the incremental source) to	the fourth snapshot (the incremental
#	    target. Save this stream into the file.
#	14. Verify that an incremental replication stream can be received
#	    after a series of promotes.
#
