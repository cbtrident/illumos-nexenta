...
for pool in $(get_pools); do
	log_note "dataset_create_write_destroy $pool >$stresslog.$child"
	dataset_create_write_destroy $pool >$stresslog.$child 2>&1 &
	child_pids="$child_pids $!"
	(( child += 1 ))

	log_note "dataset_create_write_destroy_exattr $pool > " \
		"$stresslog.$child"
	dataset_create_write_destroy_exattr $pool > \
		$stresslog.$child 2>&1 &
	child_pids="$child_pids $!"
	(( child += 1 ))
done
